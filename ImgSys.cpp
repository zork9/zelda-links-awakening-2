...
int ImgSys::load(char *filename, SDL_Surface *surface)
{
//	if ( !::strlen(filename) ) { surface = NULL; return(-1); } 
	if ( filein(filename, surface) && surface != NULL) { return(0); }
	//never reached.
	return -1;
}

int ImgSys::load2(char *filename, SDL_Surface **surface)
{
//	if ( !::strlen(filename) ) { *surface = NULL; return(-1); } 
	if ( filein2(filename, surface) && *surface != NULL) { return(0); }
	//never reached.
	return -1;
}

/*int ImgSys::load(char * filename, map<char *, SDL_Surface *> *surfaces)
{
...
*/
int ImgSys::reload(char * filename, SDL_Surface *surface)
{
...
int ImgSys::filein(char * filename, SDL_Surface *surface)
{
	SDL_Surface *ts = SDL_LoadBMP(filename);
...
int ImgSys::filein2(char * filename, SDL_Surface **surface)
{
	SDL_Surface *ts = SDL_LoadBMP(filename);
	*surface = SDL_DisplayFormat(ts);
//	SDL_FreeSurface(ts);	
